...
public class TomeBaptismListController {
	...
	@RequestMapping(value = "/tomes/baptismTome", method = RequestMethod.GET)
	public ModelAndView formBackingObject(
		...
		ModelAndView mv = new ModelAndView("tomes/baptismTome");
		Integer[] baptismYears = personManager.getBaptismYears();

		ExtendedPaginatedList baptismesList = paginateListFactory.getPaginatedListFromRequest(request);
		baptismesList.setPageSize(PAGESIZE);

		if (filter.isNotBlank()) {
			personManager.getRecPageBaptismes(baptismesList, filter.getSurname(), filter.getFirstName(), String.valueOf(filter.getYear()));
			mv.addObject(Constants.PERSONS_LIST, baptismesList);
		} else {
			personManager.getRecPageBaptismes(baptismesList, null, null, null);
			mv.addObject(Constants.PERSONS_LIST, baptismesList);
		}
		
		mv.addObject(Constants.BAP_CON_DEA_FILTER, filter);
		mv.addObject(Constants.YEARS, baptismYears);
		return mv;
	}

	@RequestMapping(value = "/tomes/baptismTome", method = RequestMethod.POST)
